...
const ACTOR_BIO = "system.details.biography.value";
const ACTOR_XP = "system.details.xp.value";
...
if ( !tokens.length ) tokens = RetrievePCTokens();
...
actors.forEach(actor => {
 const actorXP = foundry.utils.getProperty(actor, ACTOR_XP);
...
async function award_xp(totalAmount, amountArray) {
  ...
  const scene_name = canvas.scene.navName || canvas.scene.name;
  ...
  actors.forEach(actor => {
      ...
      const updates = {};
      if ( UPDATE_ACTOR_XP ) updates[ACTOR_XP] = foundry.utils.getProperty(actor, ACTOR_XP) + awardAmount;
      if ( UPDATE_JOURNAL ) {
         const currentNotes = foundry.utils.getProperty(actor, ACTOR_BIO) ?? "";
         const updatedNotes = currentNotes + `
        <p><em>${scene_name}</em> ${awardAmount} XP earned.</p>
      `
        updates[ACTOR_BIO] = updatedNotes;
      }
      if ( !foundry.utils.isEmpty(updates) ) promises.push(actor.update(updates));
  });
