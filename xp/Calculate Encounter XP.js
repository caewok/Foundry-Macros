...
function RetrieveHostileNPCTokens() {
  return canvas.tokens.placeables.filter(t => t.actor && t.document.disposition === -1 && !t.actor.hasPlayerOwner);
}
 ...
 */
function isEmptyString(str) { return (!str || /^\s*$/.test(str)); }

/**
 * Determine XP for a token
 * @param {Token} token
 * @returns {number}
 */
function tokenXP(token) { return foundry.utils.getProperty(token.actor, ACTOR_XP) || 0; }

// collect tokens
let tokens = canvas.tokens.controlled;
if ( !tokens.length ) tokens = RetrieveHostileNPCTokens();
if ( !tokens.length ) {
...
`
  <tr class="character-row">
                ...
                <td name="creature" class="creature">${t.actor.name}</td>
                <td name="creatureXP" class="creatureXP">${xp}</td> &nbsp;
                <td> <input type="number" name="adjustment" class="adjustment" value=0 min="-30000" max="30000"/></td> &nbsp;
